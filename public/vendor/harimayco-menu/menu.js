...
function addcustommenu() {
	if(($("#custom-menu-item-name").val().length === 0) || ($("#custom-menu-item-url").val().length === 0)){
...
function actualizarmenu() {
	 metadata = [];
    $('.menu-settings').find('input:checkbox').each(function () {
        var name = $(this).attr('name');
        if($(this).is(':checked')){
            metadata.push({[name]:true});
		}else{
            metadata.push({[name]:false});
        }
